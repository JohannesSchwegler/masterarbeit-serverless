...
import Material from "@/deployment-units/material-management/business-objects/material/materials.bo";
import Customer from "@/deployment-units/customer/business-objects/customer/customer.bo";
export const restoreDefaultDataHandler: APIGatewayProxyHandler =
  async (): Promise<APIGatewayProxyResult> => {
    ...
    promises.push(
      createMultiple(
        materials.splice(0, 6).map((mat) => {
          return new Material(mat).toItem();
        }),
      ),
    );
    ...
    promises.push(
      createMultiple(
        customers.splice(0, 6).map((cust) => {
          return new Customer(cust).toItem();
        }),
      ),
    );
